...
class MODSReader(Reader):
    ...
    def __init__(self, file_location):
        super(MODSReader, self).__init__(file_location, '{0}mods'.format(NAMESPACES['mods']))


class OAIReader(Reader):

    def __init__(self, file_location):
        doc_root = etree.parse(file_location).getroot().tag
        if 'repository' in doc_root:
            super(OAIReader, self).__init__(file_location, '{0}record'.format(NAMESPACES['oai_dc']))
        elif 'exportedRecords' in doc_root:
            super(OAIReader, self).__init__(file_location, '{0}record'.format(NAMESPACES['repox']))
